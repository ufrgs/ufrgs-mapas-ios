...
open class PopupController: UIViewController {
    ...
    public enum PopupLayout {
        ...
        func origin(_ view: UIView, size: CGSize = UIScreen.main.bounds.size) -> CGPoint {
            switch self {
            case .top: return CGPoint(x: (size.width - view.frame.width) / 2, y: (size.height - view.frame.height) / (6))
...
private extension PopupController {
    ...
    func registerNotification() {
        ...
        NotificationCenter.default.addObserver(self, selector: #selector(PopupController.popupControllerWillHideKeyboard(_:)), name: UIResponder.keyboardWillShowNotification, object: nil)
        NotificationCenter.default.addObserver(self, selector: #selector(PopupController.popupControllerDidHideKeyboard(_:)), name: UIResponder.keyboardWillShowNotification, object: nil)
    }
...
private extension PopupController {
    ...
    func fadeOut(_ completion: @escaping () -> Void) {
        ...
        UIView.animate(
            withDuration: 0.2, delay: 0.0, options: UIView.AnimationOptions(), animations: { () -> Void in
